...
const ImageCropOverlay = () => {
	...
	const checkCropBounds = ({
		...
		// Is the new x pos plus crop width going to exceed the right hand bound
			...
			const limitedXPos =
				imageBounds.x + imageBounds.width - cropSize.width;
			accDx = limitedXPos;
		} else {
		...
		// Is the new y pos plus crop height going to exceed the bottom bound
			...
			const limitedYPos =
				imageBounds.y + imageBounds.height - cropSize.height;
			accDy = limitedYPos;
		} else {
			// It's somewhere in between - no formatting required
		}
	...
	return (
		<GestureHandlerRootView style={tw.style("w-full h-full absolute")}>
			<PanGestureHandler
			...
			>
				<Animated.View
					style={[
						tw.style(
							"w-10 h-10 border-1 border-fans-white/58 bg-[#33333355]",
						),
						animatedCropSize,
						{
							transform: [
				...
				>
					{
						...
						horizontalSections.map((hsection) => {
							return (
								<View
									style={tw.style("flex-row flex-1")}
									key={hsection}
								>
									{verticalSections.map((vsection) => {
										...
										return (
											<View
												style={tw.style(
													"flex-1 border-[0.5px] border-fans-white/58 items-center justify-center",
												)}
												key={key}
											>
												{
													...
													key == "bottomright" ? (
														<View
															style={[
																tw.style(
																	"w-[30px] h-[30px] absolute border-white",
																),
																hsection ==
																"top"
																	? {
